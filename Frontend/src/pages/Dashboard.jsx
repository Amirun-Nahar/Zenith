...
function ExamCountdown() {
  ...
  return (
    <div>
      <h2 className="text-lg sm:text-xl font-medium mb-3 sm:mb-4" style={{ color: '#E43D12' }}>Exam Countdown</h2>
        ...
        {remaining ? (
          <div className="flex flex-col sm:flex-row items-center sm:items-end gap-3 sm:gap-4 md:gap-6 mt-4">
            <div className="text-center">
              <div className="text-3xl sm:text-4xl font-bold" style={{ color: '#E43D12' }}>{remaining.days}</div>
              <div className="text-xs sm:text-sm" style={{ color: '#E43D12', opacity: 0.7 }}>Days</div>
            </div>
            <div className="text-center">
              <div className="text-2xl sm:text-3xl font-semibold" style={{ color: '#E43D12' }}>{remaining.hours}</div>
              <div className="text-xs sm:text-sm" style={{ color: '#E43D12', opacity: 0.7 }}>Hours</div>
            </div>
            <div className="sm:ml-auto text-center sm:text-right">
              <div className="font-medium text-sm sm:text-base" style={{ color: '#E43D12' }}>{title}</div>
              <div className="text-xs sm:text-sm" style={{ color: '#E43D12', opacity: 0.7 }}>{date || 'Pick a date'}</div>
            </div>
          </div>
        ) : (
  ...
  return (
    <div className="space-y-5 sm:space-y-6 md:space-y-8">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-3 md:gap-4">
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold" style={{ color: '#E43D12' }}>Dashboard</h1>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 md:gap-6">
          <section className="card p-4 sm:p-6">
            <h2 className="text-lg sm:text-xl font-medium mb-3 sm:mb-4" style={{ color: '#E43D12' }}>Next Class</h2>
            {nextClass ? (
              <div className="space-y-2 sm:space-y-3">
                <div className="font-medium text-base sm:text-lg" style={{ color: '#E43D12' }}>{nextClass.subject}</div>
                <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm sm:text-base" style={{ color: '#E43D12', opacity: 0.7 }}>
              ...
              <p className="text-sm sm:text-base" style={{ color: '#E43D12', opacity: 0.8 }}>No upcoming classes.</p>
            )}
          </section>

          <section className="card p-4 sm:p-6">
            <h2 className="text-lg sm:text-xl font-medium mb-3 sm:mb-4" style={{ color: '#E43D12' }}>Budget Snapshot</h2>
            <div className="grid grid-cols-3 gap-2 sm:gap-3">
              <div className="text-center p-2.5 sm:p-3 rounded-lg bg-white/5">
                <div className="text-xs sm:text-sm mb-1.5 sm:mb-2" style={{ color: '#E43D12', opacity: 0.7 }}>Income</div>
                <div className="font-semibold text-sm sm:text-base md:text-lg text-lime-400">${summary.income.toFixed(2)}</div>
              </div>
              <div className="text-center p-2.5 sm:p-3 rounded-lg bg-white/5">
                <div className="text-xs sm:text-sm mb-1.5 sm:mb-2" style={{ color: '#E43D12', opacity: 0.7 }}>Expense</div>
                <div className="font-semibold text-sm sm:text-base md:text-lg text-red-300">${summary.expense.toFixed(2)}</div>
              </div>
              <div className="text-center p-2.5 sm:p-3 rounded-lg bg-white/5">
                <div className="text-xs sm:text-sm mb-1.5 sm:mb-2" style={{ color: '#E43D12', opacity: 0.7 }}>Balance</div>
